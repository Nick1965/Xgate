...
namespace SocialExplorer.IO.FastDBF
{
	...
	public class DbfRecord
	{
		...
		private static Encoding ASCIIEncoder { get; set; } // = Encoding.GetEncoding(866);
			...
			set
			{
				...
				else
				{
					...
					if (ocolType == DbfColumn.DbfColumnType.Character)
					{
						...
						// Encoding.UTF8.GetBytes(value, 0, value.Length > ocol.Length ? ocol.Length : value.Length, mData, ocol.DataAddress);
					}
					else if (ocolType == DbfColumn.DbfColumnType.Number)
					{
						...
						if (ocol.DecimalCount == 0)
						{
							...
							// Encoding.UTF8.GetBytes(value, 0, nNumLen, mData, (ocol.DataAddress + ocol.Length - nNumLen));
						}
						else
						{
							...
							if (nidxDecimal > -1)
							{
								...
								//  Encoding.UTF8.GetBytes(cDec, 0, nLen, mData, (ocol.DataAddress + ocol.Length - ocol.DecimalCount));
							}
							...
							// Encoding.UTF8.GetBytes(cNum, 0, nNumLen, mData, ocol.DataAddress + ocol.Length - ocol.DecimalCount - nNumLen - 1);
