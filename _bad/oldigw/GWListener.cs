        ...
        private void ContextReceivedCallback(IAsyncResult asyncResult)
        {
			...
			try
			{
                ...
                if (m_HttpListener != null && asyncResult != null)
                {
					...
					var ip = listenerContext?.Request?.RemoteEndPoint?.ToString();

					// Подготовка объекта RequestInfo, который будет передан в обработку
					dataHolder = new RequestInfo(listenerContext);

					var qs = "";
					foreach (var q in listenerContext.Request.QueryString.AllKeys)
						qs += $"{q} = {listenerContext.Request.QueryString[q]}; ";
					Log($"[{ip}] Получен контекст: {qs}");
			...
			{
				Log("[{0}] {1}\r\n{2}", listenerContext?.Request?.RemoteEndPoint?.Address?.ToString(), ex.Message, ex.StackTrace);
			}
